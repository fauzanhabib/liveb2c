...
class find_coaches_wa extends MY_Site_Controller {
    ...
    public function index() {
        ...
        $this->template->content->view('contents/b2c/student/find_coach_wa/index', $vars);
    ...
    public function detail($id = '') {
        ...
        $this->template->content->view('default/contents/find_coach_wa/detail', $vars);
        $this->template->publish();
    }
    ...
    public function search($category = null, $page='') {
        ...
        if ($category == 'name' || $category == null) {
            // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('b2c/student/find_coaches_wa/search/name'), count($this->identity_model->get_coach_identity(null, @$this->input->post('name'))), $per_page, $uri_segment);
            $coaches = $this->identity_model->get_coach_identity(null, @$name, null, null, null, null, null, $per_page, $offset);
        } else if ($category == 'country') {
            // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('b2c/student/find_coaches_wa/search/country'), count($this->identity_model->get_coach_identity(null, null, @$this->input->post('country'))), $per_page, $uri_segment);
            ...
            // $pagination = $this->common_function->create_link_pagination($page, $offset, site_url('b2c/student/find_coaches_wa/search/spoken_language'), count($this->identity_model->get_coach_identity(null, null, null, null, null, null, @$this->input->post('language'))), $per_page, $uri_segment);
       ...
       // print_r($coaches); exit;
        ...
        $this->template->content->view('contents/b2c/student/find_coach_wa/' . $category . '/index', $vars);
        $this->template->publish();
    }
            ...
            if ($isValid) {
                ...
                if ($this->db->trans_status() === true && $remain_token >= 0) {
                    ...
                    if ($this->db->trans_status() == 1 && $appointment_id && $valid_appointment == 1) {
                        ...
                        redirect('b2c/student/find_coaches_wa/book_by_single_date/' . date("Y-m-d", $date));
                    } else {
    ...
    public function multiple_date() {
        ...
        $this->template->content->view('default/contents/find_coach_wa/availability/multiple_date/index', $vars);
        $this->template->publish();
    }
    ...
    public function book_by_multiple_date() {
        ...
        redirect('student/find_coaches_wa/book_by_multiple_date_index/1');
    }
    ...
    public function book_by_multiple_date_index($index = '') {
        ...
        if (!$this->session->userdata('date_' . $index)) {
            ...
            redirect('student/find_coaches_wa/multiple_date');
        }
        ...
        $this->template->content->view('default/contents/find_coach_wa/book_by_availability/multiple_date/index', $vars);
        $this->template->publish();
    }
...
//        echo(date('Y-m-d', $date));
        ...
        if(!$check_max_book_coach_per_day){
            $this->messages->add('This coach has exceeded maximum booked today', 'warning');
            redirect('student/find_coaches_wa/book_by_multiple_date_index/' . $index);
        }
    ...
    public function confirm_book_by_multiple_date() {
        ...
        if (!$data_temp) {
            ...
            $this->template->content->view('default/contents/find_coach_wa/confirm_book_by_multiple_date/index', $vars);
            //publish template
            $this->template->publish();
        }
    }
...
//        $data['headers'] .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
    ...
    public function delete_temporary_appointment($id = '') {
        ...
        redirect('student/find_coaches_wa/confirm_book_by_multiple_date');
    }
    ...
    public function schedule_detail($id = '') {
        ...
        $this->template->content->view('contents/b2c/student/find_coach_wa/schedule_detail', $vars);
...
//                        break;
    ...
    public function availability($search_by = '', $coach_id = '', $date_ = '') {
        ...
        if (!$date_ || !$coach_id) {
            //redirect(home);
            $vars = array();
            $this->template->content->view('contents/b2c/student/find_coach_wa/availability', $vars);
...
//        print_r(date('Y-m-d','1450962000')); exit;
    ...
    private function isOnAvailability($coach_id = '', $date_ = '') {
        if (!$date_ || !$coach_id) {
            //redirect(home);
            $vars = array();
            $this->template->content->view('default/contents/find_coach_wa/availability', $vars);
  ...
  //        }
    ...
    public function booking($coach_id = '', $date_ = '', $start_time_ = '', $end_time_ = '', $token, $browser_type, $device_type, $device_os) {
        ...
        if(!$check_max_book_coach_per_day){
            $this->messages->add('This coach has exceeded maximum booked today', 'warning');
            redirect('b2c/student/find_coaches_wa/search/name/');
        }
...
//                exit;
                if (in_array(array('start_time' => $start_time_available, 'end_time' => $end_time_available), $availability)) {
                    ...
                    $this->messages->add('Invalid Time', 'warning');
                    redirect('student/find_coaches_wa/search/name/');
                }
                // begin the transaction to ensure all data created or modified structural
                //$token_cost = $this->coach_token_cost_model->select('token_for_student')->where('coach_id', $coach_id)->get();
                // updating remaining token student
...
//        print_r($schedule); exit;
    ...
    public function summary_book($search_by = '', $coach_id = '', $date = '', $start_time = '', $end_time = '') {
        ...
        $this->template->content->view('contents/b2c/student/find_coach_wa/summary_book/index', $vars);
        //publish template
        $this->template->publish();
    }
