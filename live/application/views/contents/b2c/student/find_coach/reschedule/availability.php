            ...
            for ($i = 0; $i < count(@$availability); $i++) {
                ...
                <tr>
                    ...
                    <?php if(($this->auth_manager->role() != "PRT") && ($this->auth_manager->role() != 'ADM') && ($this->auth_manager->role() != 'RAD')){ ?>
                        <a href="<?php echo site_url('b2c/student/manage_appointments/summary_book/'.$search_by.'/' . $appointment_id . '/' . $coach_id . '/' . strtotime(@$adate) . '/' . @$availability[$i]['start_time'] . '/' . @$availability[$i]['end_time']); ?>" class="pure-button btn-small btn-white">Book</a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
