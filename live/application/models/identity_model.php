...
//                            array('field'=>'city', 'label' => 'City', 'rules'=>'trim|required|xss_clean'),
    ...
    public function get_old_coach_identity($id = '', $fullname = '', $country = '', $partner_id = '', $date_available = '', $creator_id = '', $spoken_language='', $limit='', $offset='', $subgroup_id = ''){   
        // echo $subgroup_id;
        // exit();
...
//        print_r($fullname); //exit;
                ...
                if(($this->auth_manager->role() == 'STD') || ($this->auth_manager->role() == 'SPR') || ($this->auth_manager->role() == 'PRT')){
                    ...
                    $this->db->where_in('c.subgroup_id', $new_group_array);
                        ...
                        if($stu_rel){
                            ...
                            $group_array= array();
                            foreach(@$coregro as $cgo){
                                foreach(@$cgo as $cgo_val){
                                    if($cgo_val->subgroup_id != $user_subgroup){
                                        //$this->db->or_where('c.partner_id', $cs->coach_supplier_id);
                                        $group_array[] = $cgo_val->subgroup_id;
                                    }
                                }
                            }
                            $new_partner_array = array_keys(array_flip($partner_array));
                            $new_group_array = array_unique($group_array);

                            $count_check = count($check_array_coach);
                            $count_group = count($group_array);
                            ...
                            if($new_partner_array[0]==$partner_id){
                                for($i=0; $i < count($supplier_only); $i++){ 
                                    for($j=0; $j < count($supplier_only[$i]); $j++){
                                        if (($key = array_search(@$supplier_only[$i][$j]->coach_supplier_id, $temp)) !== true) {
                                                unset($temp[$key]);
                                            }
                                    }
                                }
                                // echo "<pre>";print_r($temp);exit();
                                $countemp = count($temp2);
                                for($i=0; $i <= $countemp; $i++){ 
                                    if (($key = array_search(@$temp[$i], $temp2)) !== false) {
                                        unset($temp2[$key]);
                                    }
                                }
                                // echo "<pre>";print_r($temp2);exit();
                                if($spe_sub){
                                    // echo "a";exit();
                                    for($i=0; $i < count($spe_sub); $i++){ 
                                        for($j=0; $j < count($spe_sub[$i]); $j++){
                                            if (($key = array_search(@$spe_sub[$i][$j]->partner_id, $temp2)) !== false) {
                                                    unset($temp2[$key]);
                                            }
                                        }
                                    }
                                }
                                $merge = array_merge($new_partner_array, $temp2);
                                $uniqe_merge = array_keys(array_flip($merge));
                                // echo "<pre>";print_r($uniqe_merge);exit();
                                $this->db->where_in('c.partner_id', $uniqe_merge);
                                if($date_available){
                                    $this->db->join('coach_dayoffs f', 'a.id = f.coach_id', 'full');
                                 }
                                 if($creator_id){
    ...
    public function get_coach_identity_27_feb_2018($id = '', $fullname = '', $country = '', $partner_id = '', $date_available = '', $creator_id = '', $spoken_language='', $limit='', $offset='', $subgroup_id = ''){
        // echo $subgroup_id;
        // exit();
        if(($this->uri->segment(1) == 'partner') && ($this->uri->segment(3) != 'coach_detail')){
           $subgroup_id = $this->uri->segment(4);
...
//        print_r($fullname); //exit;
    ...
    public function get_old_coach_identity_2($id = '', $fullname = '', $country = '', $partner_id = '', $date_available = '', $creator_id = '', $spoken_language='', $limit='', $offset='', $subgroup_id = ''){   
        // echo $id;
        // exit('a');
        if(($this->uri->segment(1) == 'partner') && ($this->uri->segment(3) != 'coach_detail')){
           $subgroup_id = $this->uri->segment(4);
...
//        print_r($fullname); //exit;
    ...
    public function get_old_coach_identity_rescedule($partner_id = '', $cert_studying = ''){       
